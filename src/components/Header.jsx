...
export default function Header({ online }) {
  ...
  const showNew = pathname !== '/ajustes'
    ...
    <header className="surface border-b sticky top-0 z-10" style={{ background: 'var(--c-surface)' }}>
      <div className="max-w-5xl mx-auto px-4 py-3 flex items-center gap-3">
        <div className="flex items-center gap-2 mr-auto min-w-0">
          ...
          <h1 className="font-bold text-lg truncate">Controle de Producao</h1>
          <span
        ...
        <ThemeToggle />
        {hasPin && (
          <button className="btn shrink-0" onClick={lock} aria-label="Bloquear app">🔒</button>
        )}
      </div>
